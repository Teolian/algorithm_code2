...
class MyAI(Alg3D):
    ...
    # --- роль-зависимые дебюты (первый/второй ход) ---
    def _opening_first(self, board: List[List[List[int]]]) -> Optional[Tuple[int, int]]:
        # Я хожу первым: центр и полуцентры приоритетнее.
        prefs = [(1,1), (2,2), (1,2), (2,1), (0,0), (3,3), (0,3), (3,0)]
        for x,y in prefs:
            if drop_z(board, x, y) is not None:
                return (x, y)
        return None

    def _opening_second(self, board: List[List[List[int]]], player: int, last_move: Tuple[int,int,int]) -> Optional[Tuple[int, int]]:
        # Я второй и делаю первый для себя ход.
        # 1) Если центр/полуцентры свободны — берём их.
        for x,y in [(1,1), (2,2), (1,2), (2,1)]:
            if drop_z(board, x, y) is not None:
                return (x, y)
        # 2) Иначе реагируем на первый ход соперника: тот же столбец, затем зеркала.
        ox, oy = (last_move[0] if last_move[0] is not None else 1,
                  last_move[1] if last_move[1] is not None else 1)
        for x,y in [(ox,oy), (3-ox,3-oy), (ox,3-oy), (3-ox,oy)]:
    ...
    def get_move(
        ...
        try:
            # --- Определяем, это мой первый ход в партии и кто вышел первым ---
            total_my = sum(1 for z in range(4) for y in range(4) for x in range(4) if board[z][y][x] == player)
            total_opp = sum(1 for z in range(4) for y in range(4) for x in range(4) if board[z][y][x] == (3 - player))
            is_my_first_turn = (total_my == 0)
            try_open = None
            if is_my_first_turn:
                if total_opp == 0:
                    # Я начинаю (первый игрок в партии)
                    try_open = self._opening_first(board)
                else:
                    # Я второй: отвечаю на дебют соперника
                    try_open = self._opening_second(board, player, last_move)
            if try_open is not None:
                return self._validate_move(board, try_open[0], try_open[1])
